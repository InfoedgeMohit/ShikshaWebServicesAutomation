...
public class GetAllSubsForUser extends Common {
	...
	public void doItBeforeTest() {
		...
		// api path
		apiPath = "sums/v1/info/getAllSubsForUser";
		...
		// test data file
		testDataFilePath = "//src//test/resources//sums//info//GetAllSubsForUser.xlsx";
	}
	...
	public void verifyGetAllSubsForUserApi(String userId, String apiResponseMsgExpected) {
	...
	public void verifyPostiveCases(JsonPath jsonPath){
		try {
			...
			ResultSet rs = exceuteDbQuery("select s.SubscriptionId, sp.SubscriptionStartDate, sp.SubscriptionEndDate, sp.TotalBaseProdQuantity, sp.BaseProdRemainingQuantity, sp.BaseProductId, bp.BaseProdCategory, bp.BaseProdSubCategory from Subscription s, Subscription_Product_Mapping sp, Base_Products bp where s.SubscriptionId = sp.SubscriptionId and sp.BaseProductId = bp.BaseProductId and s.SubscrStatus = 'ACTIVE' and sp.Status = 'ACTIVE' and sp.BaseProdRemainingQuantity >=0 and sp.SubscriptionEndDate >= curdate() and sp.SubscriptionStartDate <= now() and s.ClientUserId = "+userId+" order by sp.BaseProductId, sp.SubscriptionStartDate, sp.SubscriptionEndDate;","SUMS");
			...
			if(rowcount > 0){
				...
				// verify message
				String messageResponse = jsonPath.getString("message");
				if(messageResponse.contentEquals("This user don`t have any ACTIVE subscriptions.")) {
					// Reporter.log(", Correct message [This user don`t have any ACTIVE subscriptions.]", true);
				}
				else{
					Reporter.log(", InCorrect message ["+messageResponse+"]", true);
					Assert.fail("InCorrect message ["+messageResponse+"]");
	...
	@Test (priority = 2)
	public static void respnseTimeStats_GetAllSubsForUser(){
