...
public class getCourseDataSeatsCatWise extends Common {
	...
	boolean result;
	String file = "Logs//getCourseDataSeatsCatWise";
	...
	public void verifyGetCoursedataSearsCatWiseApi(String courseId, String cityId,
...
//		courseId = "294722";
	...
	public void verifyPostiveCases(String courseId, String cityId,
		...
		try{
			ResultSet rs = DatabaseCommon.getSeatsDateCatwise(courseId);
			if(getCountOfResultSet(rs)>0){
				...
				List<HashMap<String, String>> jsonList = new ArrayList<HashMap<String,String>>(jsonPath.getList("data.seatsData.categoryWiseSeats"));
				for(int i=0; i<jsonList.size();i++){
					...
					for(Map.Entry<String, String> entrySet: jsontempMap.entrySet()){
						...
						HashMap<String, String> dbTempMap = DBList.get(catName.toLowerCase());
						...
						if(jsonValue.equalsIgnoreCase(dbValue)){
							Reporter.log("Seat data category is appear correct for courseId: "+courseId+ " for Category: "+catName, true);
						}
						else{
							Reporter.log("Seat data category is not appear correct for courseId: "+courseId+" keyName: "+jsonkey, true);
							filewriter("Seat data category is not appear correct for courseId: "+courseId+" keyName: "+jsonkey, file);
				...
				if(jsonPath.getString("data.seatsData.categoryWiseSeats")==null){
					Reporter.log("Seat data category appear to be null when result count is 0, so Passed for courseId: "+courseId, true);
				}
				else{
					if(jsonPath.getList("data.seatsData.categoryWiseSeats").size()==0){
						Reporter.log("Seat data category appear to be null when result count is 0, so Passed for courseId: "+courseId, true);
					}
					else{
					Reporter.log("Seat data category is not appear null when result count is 0, so failed for courseId: "+courseId, true);
					filewriter("Seat data category is not appear null when result count is 0, so failed for courseId: "+courseId, file);
					filewriter("Seat Data category wise: "+jsonPath.getList("data.seatsData.categoryWiseSeats"), file);
					Assert.assertTrue(false, "Seat data category is not appear null when result count is 0, so failed for courseId: "+courseId);
					}
				}
			}
			
			ResultSet rstotalSeats = DatabaseCommon.getParentInstituteId(courseId);
			if(getCountOfResultSet(rstotalSeats)>0){
				while(rs.next()){
					int totalSeats = rs.getInt("total_seats");
					if(totalSeats==jsonPath.getInt("data.seatsData.totalSeats")){
						Reporter.log("Total Seats appear correct for courseId: "+courseId, true);
					}
					else{
						Reporter.log("Total Seats is not appear correct for courseId: "+courseId, true);
						filewriter("Total Seats is not appear correct for courseId: "+courseId, file);
						filewriter("Actual Value: "+jsonPath.getInt("data.seatsData.totalSeats")+"expected Value: "+totalSeats, file);
						Assert.assertTrue(false, "Total Seats is not appear correct for courseId: "+courseId);
					}
				}
			}
		}
	...
	@Test(priority = 2)
	public static void respnseTimeStats_getCourseDataSeatsCatWise() {
